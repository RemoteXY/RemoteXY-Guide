...
##### Где хранится состояние выключателя

Состояние выключателя хранится только в контроллере в связанной переменной и больше нигде. Состояние выключателя не хранится в графическом интерфейсе мобильного приложения. Когда мобильное приложение подключается к контроллеру, оно в первую очередь запрашивает состояние переключателя из контроллера. После этого вы можете увидеть переключатель в графическом интерфейсе в том состоянии какое значение имела связанная переменная.
...
Сохранить и восстановить состояние выключателя в EEPROM

```
uint8_t getSwitchStateFromEEPROM () {
  // TODO
}
void setSwitchStateToEEPROM (uint8_t state) {
  // TODO
}

void setup () {
  RemoteXY_Init ();  
  RemoteXY.switch = getSwitchStateFromEEPROM ();
}
...
void loop () {
  ...
  if (RemoteXY.switch != prevSwitchState) {
    setSwitchStateToEEPROM (RemoteXY.switch);   
  }
  prevSwitchState = RemoteXY.switch;
}
```
